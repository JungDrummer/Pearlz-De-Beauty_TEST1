  ...
  return (
    <>
      <nav className="fixed top-0 w-full p-6 md:p-8 flex justify-between items-center z-[100] mix-blend-difference text-white">
        ...
        {/* Desktop Links */}
        <div className="hidden md:flex gap-12 text-[10px] font-medium tracking-[0.3em] uppercase">
          {navLinks.map((link) => (
            <button 
              key={link.name} 
      ...
      >
        <div className="h-full flex flex-col items-center justify-center p-8 space-y-10">
          {navLinks.map((link, idx) => (
            ...
            >
              <span className="text-nude text-[10px] block mb-2 tracking-[0.5em] text-center uppercase font-bold">0{idx + 1}</span>
              {link.name}
            </button>
          ))}
          
          <div className="pt-12 border-t border-white/10 w-full flex justify-center gap-8">
            <a href="#" className="text-white/40 text-[10px] tracking-widest uppercase font-bold">Instagram</a>
            <a href="#" className="text-white/40 text-[10px] tracking-widest uppercase font-bold">Kakao</a>
          </div>
        </div>
      </div>
    </>
